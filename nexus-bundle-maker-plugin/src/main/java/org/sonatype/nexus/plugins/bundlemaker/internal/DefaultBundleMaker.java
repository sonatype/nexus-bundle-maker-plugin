...
import static org.sonatype.nexus.plugins.bundlemaker.internal.BundleMakerUtils.isHidden;
                                ...
                                final boolean forceRegeneration )
    {
        ...
        try
        {
            ...
            if ( forceRegeneration )
            {
                new SerialScanner().scan( scanPath, new ListenerSupport()
                {
                    ...
                    public void onFile( final File file )
                    {
                        ...
                        if ( !isHidden( path ) && isARecipe( path ) )
                        {
                            try
                            {
            ...
            new SerialScanner().scan( scanPath, new ListenerSupport()
            {
                ...
                {
                    final String path = getRelativePath( localStorage, file );
                    if ( !isHidden( path ) && isAJar( path ) && !isABundle( path ) )
                    {
                        try
                        {
                        ...
                        {
                            logger.warn(
                                String.format( "OSGi version of jar [%s] not created due to [%s]", path,
                                               e.getMessage() ),
                                e );
                        }
                    }
